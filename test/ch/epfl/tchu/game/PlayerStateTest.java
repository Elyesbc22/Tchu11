...
class PlayerStateTest {
    ...
    void possibleAdditionalCardsWithWrongArguments() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            ps.possibleAdditionalCards(1, createCardsBag(Card.BLACK, 3, Card.YELLOW, 2));
        });
    }
    ...
    @Test
    void canClaimRouteTest() {
        assertTrue(ps.canClaimRoute(new Route("AT2_VAD_1", DAV, LUG, 1, Route.Level.UNDERGROUND, Color.RED)));
        PlayerState playerState = new PlayerState(new SortedBag.Builder<Ticket>().build(), new SortedBag.Builder<Card>().build(), List.of());
        assertFalse(playerState.canClaimRoute(ALL_ROUTES.get(11)));
