...
//        assertEquals(Constants.ALL_CARDS.size() - 8, gameState.cardState().totalSize());
    ...
    @Test
    void playerStateAndCurrentPlayerStateAreImmutable() {
        GameState gameState = GameState.initial(SortedBag.of(tickets()), new Random());
        assertNotEquals(gameState.currentPlayerState(), gameState.playerState(gameState.currentPlayerId()));
    }

    @Test
    void topTicketsThrowsIllegalArgumentExcepetion() {
        GameState gameState = GameState.initial(SortedBag.of(tickets()), new Random());
    ...
    @Test
    void withDrawnFaceUpCardThrowsIllegalArgumentException() {
        GameState gameState = GameState.initial(SortedBag.of(tickets()), new Random());
        for (int i = 0; i < 96; ++i) {
            gameState = gameState.withoutTopCard();
        }

        final GameState gameStateWithNotEnoughCards = gameState;

        assertThrows(IllegalArgumentException.class, () -> {
            gameStateWithNotEnoughCards.withDrawnFaceUpCard(1);
        });
    }
